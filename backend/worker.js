...
async function sha256(text) {
  ...
  return [...new Uint8Array(hash)].map((b2) => b2.toString(16).padStart(2, "0")).join("");
}
  ...
  async fetch(req, env) {
    ...
    try {
      ...
      if (path === "/api/v1/auth/login" && req.method === "POST") {
        ...
        if (!/^\d{4}$/.test(pin)) return bad("El PIN debe tener 4 n\xFAmeros", 400);
        const users = await sql`
          ...
          LIMIT 1
        `;
        if (!users.length) return bad("PIN inv\xE1lido", 401);
      ...
      if (path === "/api/v1/admin/users" && req.method === "POST") {
        ...
        if (!/^\d{4}$/.test(pin)) return bad("El PIN debe tener 4 n\xFAmeros");
        const rows = await sql`
